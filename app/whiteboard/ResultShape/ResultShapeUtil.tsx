...
export class ResultShapeUtil extends BaseBoxShapeUtil<ResultShape> {
	...
	override component(shape: ResultShape) {
		...
		const handleExpand = () => {
			...
			if (nowShape?.meta.hasExpand) {
				editor.createShape(nowShape.meta.expandShape)
				if (nowShape.meta.childShapes.length) {
					nowShape.meta.childShapes.forEach((ele) => {
						editor.createShape(ele)
					})
				}
				...
				editor.createShape({
					...
					x: selectionBounds.x,
					y: selectionBounds.y,
					meta: { expandShape: shape, formResult: true },
					props: {
						w: 500,
						h: 600,
